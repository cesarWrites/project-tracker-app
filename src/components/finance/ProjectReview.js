...
const ProjectReview = () => {
  ...
  useEffect(() => {
    // Fetch submitted forms from the backend when the component mounts
    fetchSubmittedForms();
  }, []);

  const fetchSubmittedForms = async () => {
    try {
      const response = await axios.get('/api/submitted-forms');
  ...
  const approveProject = async (projectId) => {
    try {
      await axios.post(`/api/approve-project/${projectId}`);
      // Refresh the list of submitted forms after approval
      fetchSubmittedForms();
    } catch (error) {
  ...
  const requestMoreDetails = async (projectId, comment) => {
    try {
      await axios.post(`/api/request-details/${projectId}`, { comment });
      // Refresh the list of submitted forms after requesting more details
      fetchSubmittedForms();
    } catch (error) {
